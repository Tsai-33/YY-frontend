...
export default function InboundTable({ data,selectedArray, setSelectedArray }) {
  ...
  const { orderCode, step } = useSelector((s) => s.inbound[currentStationSafe] || {});
  ...
  const handleSelectedOption = (name, value, idKey) => {
    ...
    if (name === "checkbox") {
      ...
      dispatch(setInbound({ station: currentStation, orderCode: valueId, step: 2 }));
    }
  };
  ...
  return (
    <>
      {step <= 2 && <NoCheckBoxTable headers={tableHeader} data={data} type="radio" name="inbound" variants="green" idKey="orderId" checked={orderCode} onChange={handleSelectedOption} />}
      {step > 2 && <Table headers={tableHeader2} data={tableData2} type="checkbox" name="inbound2" variants="green" idKey="orderId" checked={selectedArray} onChange={handleSelectedOption} />}
    </>
  );
}
