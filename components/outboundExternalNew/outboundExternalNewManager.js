...
import { managerOutboundExternalNew, resetOutboundExternalNew, updateLackStation, updateOrderList } from "@/redux/reducer/reducerOutboundExternalNew";
import ActionBtn from "../common/btns/actionBtn";
import Alert from "../common/alert/alert";

export default function OutboundExternalNewManager({ isOpen, onClose }) {
  const dispatch = useDispatch();

  const { stations, currentStation } = useSelector((s) => s.workstation);
  const currentStationSafe = currentStation || stations?.[0] || "B01";

  const outboundExternalNewState = useSelector((state) => state.outboundExternalNew);
  const { orderList, lackStation } = outboundExternalNewState;
  const { step, screen, orderCode, waveNo, order, shelf, shelfItem, selected, selectedShelves } = useSelector((state) => state.outboundExternalNew[currentStationSafe] || {});

  const handleChange = (e) => {
    const name = e.target.name;
    const value = e.target.value;
    dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: name, value: value }));
  };
  ...
  const handleClearStation = () => {
    Alert({
      ...
      showCancel: true,
      onConfirm: () => {
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "step", value: 1 }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "screen", value: "idle" }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "orderCode", value: "" }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "waveNo", value: null }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "order", value: {} }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "shelf", value: {} }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "shelfItem", value: [] }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "selected", value: [] }));
        dispatch(managerOutboundExternalNew({ station: currentStationSafe, name: "selectedShelves", value: [] }));
      },
    });
  };
  ...
  const handleClearAll = () => {
    Alert({
      ...
      showCancel: true,
      onConfirm: () => {
        dispatch(resetOutboundExternalNew());
      },
    });
  };
      ...
      <div className="p-4 bg-white rounded-xl shadow-lg w-[60vw] max-h-[80vh] flex flex-col overflow-hidden">
        <div className="sticky top-0 bg-white z-10 border-b p-2">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold">出庫控制面板(New) - {currentStationSafe}</h2>
          ...
          <div className="mb-4 p-3 bg-yellow-50 rounded">
            ...
            <div className={`grid gap-2 ${stations?.length === 4 ? "grid-cols-4" : stations?.length === 5 ? "grid-cols-5" : "grid-cols-5"}`}>
              {stations?.map((station) => {
                const stationData = outboundExternalNewState[station] || {};
                return (
                  <div
                    key={station}
