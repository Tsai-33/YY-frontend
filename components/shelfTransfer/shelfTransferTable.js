          ...
          <div className="w-1/2 flex flex-col h-[76vh]">
            <div>
              <div className="flex items-center gap-3 mb-3">
                <label className="text-lg font-medium whitespace-nowrap">訂單單號：</label>
                <InputFrame type="text" name="orderNo" className="border-2 rounded px-4 py-2 w-[400px]" value={orderInput} onChange={handleInputChange} placeholder="請輸入訂單單號" />
              </div>
            </div>
        ...
        <div className="flex gap-2">
          {stations.map((station, index) => (
            <div key={station} className="flex-1">
              <ActionBtn text={`站點${index + 1}`} variant="green" disabled={false} className="w-full flex justify-center" />
            </div>
          ))}
        </div>
      </div>
