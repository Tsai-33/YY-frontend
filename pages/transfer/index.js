...
export default function Transfer() {
  ...
  const handleBarCode = (e) => {
    ...
    if (result) {
      dispatch(setTransfer({ station: currentStation, orderCode: inputBarCode, step: 2 }));
  ...
  const handleConfrim = async () => {
    ...
    try {
      ...
      // if(res.data.success){
      dispatch(setTransfer({ station: currentStation, screen: "loading", orderList: orderCode }));
  ...
  return (
    <>
      {/* 頂部區域 */}
      {step === 1 && <PageHeader title={`請點擊清單內的調撥單號或掃調撥單條碼`} backTo="/workspace" close={false} />}
      ...
      <div className="flex flex-1 gap-4 px-2 py-8 items-stretch">
        {/* 左側 */}
        <div className="w-3/7">
          <TransferTable data={tableData} selectedArray={selectedArray} setSelectedArray={setSelectedArray} />
        </div>
        {/* 右側 */}
