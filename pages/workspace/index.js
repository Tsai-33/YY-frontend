...
export default function WorkspaceIndex() {
  ...
  const { jobs, stations, currentStation } = useSelector((state) => state.workstation);
  ...
  const handleSelectJob = async (job) => {
    ...
    if (task?.success) {
      const hasTask = task?.data?.data?.some((item) => item.location === job.key || item.location === "");
      if (hasTask || job.key === "stockQuery") {
        // job.key
