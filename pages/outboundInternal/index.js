...
export default function OutboundInternal() {
  ...
  }, [JSON.stringify(stations.map(s => outboundInternalState[s]?.pushButton))]);
  ...
  const handleReturnShelfByStation = async (stationId) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        outboundInternalStateRef.current = {
          ...currentRefState,
          [stationId]: {
  ...
  const handleReturnShelf = async () => {
    ...
    setLoading(true);
    try {
      const noItemsSelected = (selectedArray || []).length === 0;
      ...
      const itemsToShift = selectedArray || [];
