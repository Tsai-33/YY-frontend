...
export default function ShelfTransferShelf() {
      ...
      setSelectedRows((prev) => {
        if (prev.includes(shelveId)) {
          ...
          if (prev.length >= stations.length) {
            Alert({ html: `最多只能選擇${stations.length}個貨架` });
            return prev;
          }
  ...
  const handleConfirm = async () => {
    if (!canConfirm) {
      Alert({ html: `請選擇1~${stations.length}個貨架` });
    }
    ...
    <>
      <div className="flex flex-col h-screen p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-2xl font-bold text-center flex-1">{tableData.length > 0 ? `請在左側清單內勾選最多${stations.length}個貨架，點擊確定按鈕系統會派發無人車將貨架搬運至工作站` : "請先選擇庫區後輸入品項號，點擊檢視按鈕查詢要調整的貨架"}</div>
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="flex items-center gap-2">
            ...
            <select value={selectedArea} onChange={(e) => setSelectedArea(e.target.value)} className="border-2 border-gray-300 rounded px-3 py-2 min-w-[120px]">
          ...
          <div className="flex items-center gap-2">
            ...
            <InputFrame type="text" ref={prtNoRef} onKeyDown={handleSearch} disabled={searching} placeholder="" className="w-[250px]" />
            {searching && <span className="ml-2">查詢中...</span>}
          </div>
          <ActionBtn icon="icon-delete" text="清除" variant="gray" onClick={handleClear} />
        </div>
        {/* 主要內容區 */}
        <div className="flex gap-4 flex-1">
          <div className="w-2/5">
            <Table variant="green" type="checkbox" name="shelfAdjustList" headers={headers} data={tableData} needInput={true} idKey="SHELVE_ID" height="65vh" checked={selectedRows} onChange={handleRowSelect} />
          </div>
          {/* 詳細資訊 */}
          <div className="w-3/5 bg-gray-50 rounded-lg p-6 flex flex-col h-[70vh]">
            {groupedShelveData.length > 0 ? (
                    ...
                    {shelveGroup.items.map((item, itemIndex) => (
                      <div key={itemIndex} className="border-t border-[#c4a57b] pt-3 mt-3 first:border-t-0 first:pt-0 first:mt-0">
                        ...
                        <div className="flex gap-12">
                          ...
                          <div className="text-2xl font-bold">包數：{item.BOX_PACK} 包</div>
                        </div>
                      </div>
                    ))}
              ...
              <div className="flex-1 flex items-center justify-center h-full text-gray-400 text-2xl">{tableData.length > 0 ? "請在左側勾選貨架查看詳細資訊" : "請先搜尋產品品號"}</div>
            )}
          </div>
        </div>
        {/* 按鈕區 */}
        ...
        <div className="flex gap-2">
          {stations.map((station, index) => (
            <button key={station} className="flex-1 bg-green-600 text-white py-3 rounded-lg text-lg font-medium">
              站點 {index + 1}
            </button>
          ))}
        </div>
